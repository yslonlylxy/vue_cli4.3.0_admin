	...
	export default {
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                    	if(this.$route.query.id){
		                    ...
		                    this.articleForm.cover = this.articleForm.imageUrl;
		                    this.articleForm.id = this.$route.query.id || 'null';
		                    this.$axios.post('/article/upDataArticle', this.articleForm,
			                    result => {
				                    if(result.data.code === 200){
					                    this.$message.success(result.data.msg);
					                    let menu = this.$store.state.menuList[3].children[0];
            ...
            query(){
                ...
                this.$axios.get('/article/getArticle',{id:index},
                    result => {
                        if(result.status === 200){
                            this.articleForm = result.data.data[0];
                            this.articleForm.imageUrl = this.articleForm.cover;
                            this.$store.commit('setMsg',this.articleForm.content);
                        }
                    })
            },
        },
